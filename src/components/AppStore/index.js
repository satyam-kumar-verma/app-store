...
class AppStore extends Component {
  ...
  filteredAppItemBasedOnSearchBar = () => {
    const {activeTab, searchBarText} = this.state
    const filteredItems = appsList.filter(
      eachApp => eachApp.category === activeTab,
    )
    const filteredList = filteredItems.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchBarText.toLowerCase()),
    )
    return filteredList
  }
  ...
  render() {
    ...
    const searchBarFilteredAppList = this.filteredAppItemBasedOnSearchBar()
    return (
      <div className="bg-container">
        ...
        <ul className="app-container">
          {searchBarFilteredAppList.map(eachApp => (
